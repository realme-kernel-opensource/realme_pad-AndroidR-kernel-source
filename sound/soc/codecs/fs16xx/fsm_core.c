...
int fsm_swap_channel(fsm_dev_t *fsm_dev, int next_angle)
{
    ...
    ret |= fsm_reg_write(fsm_dev,REG(FSM_DACCTRL), 0x0310);
    ...
    ret |= fsm_reg_write(fsm_dev,REG(FSM_DACCTRL), 0x0010);
...
int fsm_config_vol(fsm_dev_t *fsm_dev)
{
    ...
    if (fsm_dev->re25_dft == 0 && !fsm_dev->state.calibrated) {
        volume = 0xDF; // -12dB
    } else if (fsm_dev->state.bypass_dsp) {
        volume = 0xE7;
    } else {
        volume = cfg->volume;
    }
...
void fsm_set_scene(int scene)
{
    ...
    //fsm_mutex_lock();
    switch (1 << scene) {
      case FSM_SCENE_MMI_ALL:
          pr_info("recover all bypass");
          cfg->bypass_dsp = 0;
          break;
      case FSM_SCENE_MMI_ALL_BYPASS:
          pr_info("bypass all");
          cfg->bypass_dsp = 1;
          cfg->next_scene = BIT(scene);
          break;
      case FSM_SCENE_TOP_LEFT_BYPASS:
      case FSM_SCENE_TOP_RIGHT_BYPASS:
      case FSM_SCENE_BOT_LEFT_BYPASS:
      case FSM_SCENE_BOT_RIGHT_BYPASS:
          pr_info("speaker single bypass");
          if (cfg->speaker_on == true) {
              pr_info("speaker is on, turn it off");
              fsm_speaker_off();
              //cfg->next_scene = BIT(scene);
              //fsm_speaker_onn();
          }/* else {
          ...
          break;
      case FSM_SCENE_TOP_LEFT:
      case FSM_SCENE_TOP_RIGHT:
      case FSM_SCENE_BOT_LEFT:
      case FSM_SCENE_BOT_RIGHT:
          pr_info("spk single test");
          if (cfg->speaker_on == true) {
              pr_info("speaker is on, turn it off");
              fsm_speaker_off();
              //cfg->next_scene = BIT(scene);
...
int fsm_test_result(struct fsm_cal_result *result, int size)
{
    ...
    for (dev = 0; dev < result->ndev; dev++) {
        ...
        pr_info("dev[%d] error code: %d", dev, result->info[dev].errcode);
        if (fsm_dev->tdata == NULL || cfg->nondsp_mode) {
            if (fsm_dev->tdata == NULL) {
                pr_addr(info, "there is something wrong with 0x%02x", fsm_dev->addr);
                memset(fsm_dev->zmdata, 0, FSM_F0_COUNT * sizeof(uint16_t));
        ...
        if (!cfg->stop_test) {
            pr_addr(info, "test not finished");
