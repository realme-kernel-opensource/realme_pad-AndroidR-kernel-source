...
#define FSM_POS_MONO (0x3)
#define FSM_POS_LTOP BIT(0)
#define FSM_POS_RBTM BIT(1)
#define FSM_POS_LBTM BIT(2)
#define FSM_POS_RTOP BIT(3)
#else
...
enum fsm_livedata_mask {
    LD_LMT_GAIN = 0,   // Gain
    LD_AGC_GAIN,       // AGC
    LD_EXCURSION_DATA, // Excursion
    LD_DATA_V,         // Data V
    LD_DATA_I,         // Data I
    LD_R0,             // R0
    LD_F0,             // F0
    LD_TEMPR,          // Temperature
    LD_AEC,            // AEC
    LD_DEBUG,          // for debug purpose
    LD_MAX,
};
...
#define MAX_PRESET_FILE_SIZE  (1024*8) // In bytes
#define FSADSP_PRESET_VERSION 0xFA01 // In bytes
struct algo_preset_hdr {
    uint16_t preset_ver;
    uint16_t preset_rev_id;
    char project[8];
    char customer[8];
    int32_t create_time;     /* Time tick when the preset created */
    uint16_t data_len;       /* Length of the whole preset file in bytes */
    uint16_t crc16;          /* CRC check sum of the data */
    // Here goes with module parameters
    uint8_t data[];
};
typedef struct algo_preset_hdr algo_preset_hdr_t;
