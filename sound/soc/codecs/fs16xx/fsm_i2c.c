            ...
            const struct i2c_device_id *id)
{
    ...
    pr_info("%s:enter i2c->addr = 0x%x", __func__, i2c->addr);
    ...
    if (i2c->addr == 0x44) {
        i2c->addr = 0x34;
        pr_info("%s:enter i2c->addr = 0x%x", __func__, i2c->addr);
    } else if (i2c->addr == 0x45) {
        i2c->addr = 0x35;
        pr_info("%s:enter i2c->addr = 0x%x", __func__, i2c->addr);
    } else if (i2c->addr == 0x46) {
        i2c->addr = 0x36;
        pr_info("%s:enter i2c->addr = 0x%x", __func__, i2c->addr);
    } else if (i2c->addr == 0x47) {
        i2c->addr = 0x37;
        pr_info("%s:enter i2c->addr = 0x%x", __func__, i2c->addr);
    }
    ...
    set_smartpa_type("fs1894su", strlen("fs1894su"));
