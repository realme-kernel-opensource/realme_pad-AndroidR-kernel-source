...
#include "../audio_info/smartpa-info.h"
/******************************************************
 *
 * Marco
 *
...
#define AW881XX_DRIVER_VERSION "v0.2.6"
...
static unsigned int g_spin_switch = AW_SPIN_SWITCH_ON;
static const char *const aw881xx_spin[] = {"spin_0", "spin_90",
					"spin_180", "spin_270"};
...
static void aw881xx_fade_out(struct aw881xx *aw881xx)
{
	...
	for (i = start_volume; i <= AW_FADE_OUT_TARGET_VOL; i += g_fade_step) {
		if (i > (int32_t)AW_FADE_OUT_TARGET_VOL)
			i = (int32_t)AW_FADE_OUT_TARGET_VOL;
				...
				uint8_t *data, uint32_t len)
{
	...
	for (i = 0; i < len; i += 4) {
		...
		if (reg_addr == AW881XX_REG_I2SCTRL) {
			if (g_spin_switch) {
				reg_val &= AW881XX_BIT_I2SCTRL_CHS_MASK;
				mutex_lock(&g_aw881xx_lock);
				reg_val |= aw881xx->spin_table[g_spin_angle].rx_val;
				mutex_unlock(&g_aw881xx_lock);
			}
		}

		if (reg_addr == AW881XX_REG_I2SCFG1) {
			/*colse tx*/
			...
			if (g_spin_switch) {
				...
				mutex_lock(&g_aw881xx_lock);
				reg_val |= aw881xx->spin_table[g_spin_angle].tx_val;
				mutex_unlock(&g_aw881xx_lock);
			}
		}
...
static int aw881xx_fw_update(struct aw881xx *aw881xx, bool up_dsp_fw_en)
{
	...
	dsp_st = (aw881xx->dsp_cfg == AW881XX_DSP_WORK) ? true : false;
	aw881xx_dsp_enable(aw881xx, dsp_st);
...
static int aw881xx_set_channal_mode(struct aw881xx *aw881xx,
...
static int aw881xx_set_spin_angle(struct aw881xx *aw881xx, uint32_t spin_angle)
{
	struct aw881xx *local_dev = NULL;
	struct list_head *pos = NULL;
	int ret;

	if (spin_angle >= ARRAY_SIZE(aw881xx_spin)) {
	...
	if (g_spin_angle == spin_angle) {
		pr_info("%s: spin_angle[%s] no change\n",
			__func__, aw881xx_spin[spin_angle]);
		return 0;
	}

	list_for_each(pos, &g_dev_list) {
		local_dev = container_of(pos, struct aw881xx, list_node);
		ret = aw881xx_set_channal_mode(local_dev, local_dev->spin_table[spin_angle]);
		if (ret < 0) {
...
static void aw881xx_get_spin_angle(uint32_t *spin_value)
{
	*spin_value = g_spin_angle;
}

static int aw881xx_set_spin(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	ret = aw881xx_set_spin_angle(aw881xx, ctrl_value);
	if (ret < 0)
...
static int aw881xx_get_spin(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	mutex_lock(&g_aw881xx_lock);
	aw881xx_get_spin_angle(&ctrl_value);
	ucontrol->value.integer.value[0] = ctrl_value;
	mutex_unlock(&g_aw881xx_lock);

	aw_dev_dbg(aw881xx->dev, "%s:get spin:%s\n", __func__, aw881xx_spin[ctrl_value]);

	return 0;
}

static const struct soc_enum aw881xx_snd_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(aw881xx_spin), aw881xx_spin),
};

static struct snd_kcontrol_new aw881xx_spin_controls[] = {
	SOC_ENUM_EXT("aw881xx_spin_switch", aw881xx_snd_enum[0],
		aw881xx_get_spin, aw881xx_set_spin),
};
...
static void aw881xx_add_codec_controls(struct aw881xx *aw881xx)
{
	...
	if (aw881xx->channel == 0) {
		...
		/*add spin control*/
		if (g_spin_switch == AW_SPIN_SWITCH_ON) {
			aw_componet_codec_ops.aw_snd_soc_add_codec_controls(aw881xx->codec,
					aw881xx_spin_controls, ARRAY_SIZE(aw881xx_spin_controls));
		}
	}
}
...
static int aw881xx_add_widgets(struct aw881xx *aw881xx)
{
	...
	for (i = 0; i < ARRAY_SIZE(aw881xx_dapm_widgets); i++) {
		if (aw_widgets[i].name) {
			ret = aw881xx_append_suffix("%s_%x_%x", &aw_widgets[i].name, aw881xx);
			if (ret < 0) {
		...
		if (aw_widgets[i].sname) {
			ret = aw881xx_append_suffix("%s_%x_%x", &aw_widgets[i].sname, aw881xx);
			if (ret < 0) {
				aw_dev_err(aw881xx->dev, "aw_route source  append i2c suffix failed!\n");
				return ret;
			}
	...
	for (i = 0; i < ARRAY_SIZE(aw881xx_audio_map); i++) {
		if (aw_route[i].sink) {
			ret = aw881xx_append_suffix("%s_%x_%x", &aw_route[i].sink, aw881xx);
			if (ret < 0) {
				aw_dev_err(aw881xx->dev, "aw_route source  append i2c suffix failed!\n");
				return ret;
			}
		...
		if (aw_route[i].source) {
			ret = aw881xx_append_suffix("%s_%x_%x", &aw_route[i].source, aw881xx);
			if (ret < 0) {
				aw_dev_err(aw881xx->dev, "aw_route source  append i2c suffix failed!\n");
				return ret;
			}
...
static void aw881xx_parse_spin_table_dt(struct aw881xx *aw881xx,
					struct device_node *np)
{
	int ret = -1;
	...
	ret = of_property_read_string(np, "spin_data", &str_data);
	if (ret < 0) {
	...
	for (i = 0; i < AW_SPIN_MAX; i++) {
		if (spin_table_str[i] == 'l' || spin_table_str[i] == 'L') {
			aw881xx->spin_table[i].rx_val = AW881XX_BIT_I2SCTRL_CHS_RX_LEFT;
			aw881xx->spin_table[i].tx_val = AW881XX_BIT_I2SCFG1_CHS_TX_LEFT;
		} else if (spin_table_str[i] == 'r' || spin_table_str[i] == 'R') {
			aw881xx->spin_table[i].rx_val = AW881XX_BIT_I2SCTRL_CHS_RX_RIGHT;
			aw881xx->spin_table[i].tx_val = AW881XX_BIT_I2SCFG1_CHS_TX_RIGHT;
		} else {
			aw_dev_err(aw881xx->dev, "%s: unsupported str:%s, close spin function\n",
				__func__, str_data);
			goto error;
...
error:
	g_spin_switch = AW_SPIN_SWITCH_OFF;
}
				...
				struct device_node *np)
{
	...
	aw881xx_parse_spin_table_dt(aw881xx, np);
				...
				const struct i2c_device_id *id)
{
	...
	aw881xx->fw_status = AW881XX_FW_FAILED;
	INIT_LIST_HEAD(&aw881xx->list_node);
	...
	set_smartpa_type("aw88194a", strlen("aw88194a"));
