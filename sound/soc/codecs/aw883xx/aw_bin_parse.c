...
extern int aw_monitor_parse_fw_aw883xx(struct aw_monitor_desc *monitor_desc,
 ...
 *
 ********************************************************/
int aw_check_sum_aw883xx(struct aw_bin *bin, int bin_num)
{
	unsigned int i = 0;
...
int aw_check_data_version_aw883xx(struct aw_bin *bin, int bin_num)
{
	int i = 0;
...
int aw_check_register_num_v1_aw883xx(struct aw_bin *bin, int bin_num)
{
...
int aw_check_dsp_reg_num_v1_aw883xx(struct aw_bin *bin, int bin_num)
{
...
int aw_check_soc_app_num_v1_aw883xx(struct aw_bin *bin, int bin_num)
{
 ...
 *
 ********************************************************/
void aw_get_single_bin_header_1_0_0_aw883xx(struct aw_bin *bin)
{
	int i;
...
int aw_parse_each_of_multi_bins_1_0_0_aw883xx(unsigned int bin_num, int bin_serial_num,
				      struct aw_bin *bin)
{
	int ret = 0;
	...
	ret = aw_parse_bin_header_1_0_0_aw883xx(bin);
	return ret;
}
 ...
 */
int aw_get_multi_bin_header_1_0_0_aw883xx(struct aw_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	aw_get_single_bin_header_1_0_0_aw883xx(bin);

	for (i = 0; i < bin_num; i++) {
		DBG("aw_bin_parse enter multi bin for is %d\n", i);
		ret = aw_parse_each_of_multi_bins_1_0_0_aw883xx(bin_num, i, bin);
		if (ret < 0)
			return ret;
	}
	return 0;
 ...
 *
 ********************************************************/
int aw_parse_bin_header_1_0_0_aw883xx(struct aw_bin *bin)
{
	int ret = 0;
	...
	case DATA_TYPE_MULTI_BINS:
		...
		ret = aw_get_multi_bin_header_1_0_0_aw883xx(bin);
		if (ret < 0)
			return ret;
		break;
	}
...
int aw_parsing_bin_file_aw883xx(struct aw_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	for (i = 0; i < bin->all_bin_parse_num; i++) {
		/* check sum */
		ret = aw_check_sum_aw883xx(bin, i);
		if (ret < 0) {
			DBG_ERR("aw_bin_parse check sum data error\n");
			return ret;
		}
		/* check bin data version */
		ret = aw_check_data_version_aw883xx(bin, i);
		if (ret < 0) {
		...
		if (bin->header_info[i].bin_data_ver == DATA_VERSION_V1) {
			...
			if (bin->header_info[i].bin_data_type ==
							DATA_TYPE_REGISTER) {
				ret = aw_check_register_num_v1_aw883xx(bin, i);
				if (ret < 0)
					return ret;
				/* check dsp reg num */
			} else if (bin->header_info[i].bin_data_type ==
							DATA_TYPE_DSP_REG) {
				ret = aw_check_dsp_reg_num_v1_aw883xx(bin, i);
				if (ret < 0)
					return ret;
				/* check soc app num */
			} else if (bin->header_info[i].bin_data_type ==
							DATA_TYPE_SOC_APP) {
				ret = aw_check_soc_app_num_v1_aw883xx(bin, i);
				if (ret < 0)
					return ret;
			} else {
		...
		uint8_t *data, uint32_t data_len, struct aw_prof_desc *prof_desc)
{
	...
	ret = aw_parsing_bin_file_aw883xx(aw_bin);
	if (ret < 0) {
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (cfg_dde[i].data_type == ACF_SEC_TYPE_MONITOR) {
				ret = aw_monitor_parse_fw_aw883xx(&aw_dev->monitor_desc,
							...
							struct aw_cfg_dde_v_1_0_0_0 *cfg_dde, int *cur_scene_id)
{
	...
	case ACF_SEC_TYPE_MONITOR:
		return aw_monitor_parse_fw_aw883xx(&aw_dev->monitor_desc,
						(uint8_t *)prof_hdr + cfg_dde->data_offset,
						cfg_dde->data_size);
	default:
