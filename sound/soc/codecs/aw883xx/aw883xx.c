...
#define AW883XX_DRIVER_VERSION "v0.7.0"
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (aw883xx->aw_pa->dsp_cfg == AW_DEV_DSP_BYPASS) {
		...
		mutex_lock(&aw883xx->i2c_lock);
		aw_dev_info(aw883xx->dev, "dsp_firmware:");
		aw883xx_i2c_write(aw883xx, aw883xx->aw_pa->dsp_mem_desc.dsp_madd_reg,
			aw883xx->aw_pa->dsp_mem_desc.dsp_fw_base_addr);
		for (i = 0; i <aw883xx->aw_pa->dsp_fw_len; i += 2) {
			aw883xx_i2c_read(aw883xx, aw883xx->aw_pa->dsp_mem_desc.dsp_mdat_reg, &reg_val);
			dsp_info_len += snprintf(dsp_reg_info + dsp_info_len, AW_NAME_BUF_MAX - dsp_info_len,
				"%02x,%02x,", (reg_val >> 0) & 0xff,
						(reg_val >> 8) & 0xff);
			if ((i / 2 + 1) % 8 == 0) {
				aw_dev_info(aw883xx->dev, "dsp_fw: %s", dsp_reg_info);
				dsp_info_len = 0;
				memset(dsp_reg_info, 0, AW_NAME_BUF_MAX);
			}

			if (((aw883xx->aw_pa->dsp_fw_len) % 8 != 0) &&
				(i == (aw883xx->aw_pa->dsp_fw_len - 2))) {
				aw_dev_info(aw883xx->dev, "dsp_fw: %s", dsp_reg_info);
				dsp_info_len = 0;
				memset(dsp_reg_info, 0, AW_NAME_BUF_MAX);
			}
		...
		aw_dev_info(aw883xx->dev, "dsp_config:");
		aw883xx_i2c_write(aw883xx, aw883xx->aw_pa->dsp_mem_desc.dsp_madd_reg,
			aw883xx->aw_pa->dsp_mem_desc.dsp_cfg_base_addr);
		for (i = 0; i <aw883xx->aw_pa->dsp_cfg_len; i += 2) {
			aw883xx_i2c_read(aw883xx, aw883xx->aw_pa->dsp_mem_desc.dsp_mdat_reg, &reg_val);
			...
			if ((i / 2 + 1) % 8 == 0) {
				aw_dev_info(aw883xx->dev, "dsp_cfg: %s", dsp_reg_info);
				dsp_info_len = 0;
				memset(dsp_reg_info, 0, AW_NAME_BUF_MAX);
			}

			if (((aw883xx->aw_pa->dsp_cfg_len) % 8 != 0) &&
				(i == (aw883xx->aw_pa->dsp_cfg_len - 2))) {
				aw_dev_info(aw883xx->dev, "dsp_cfg: %s", dsp_reg_info);
				dsp_info_len = 0;
				memset(dsp_reg_info, 0, AW_NAME_BUF_MAX);
			}
				...
				const struct i2c_device_id *id)
{
	...
	if(aw883xx->i2c->addr==0x38){
		aw883xx->i2c->addr=0x34;
		aw_dev_err(&i2c->dev, "aw883xx i2c add is from 0x38 to 0x34");
	}
	if(aw883xx->i2c->addr==0x39){
		aw883xx->i2c->addr=0x35;
		aw_dev_err(&i2c->dev, "aw883xx i2c add is from 0x39 to 0x35");
	}
	if(aw883xx->i2c->addr==0x40){
		aw883xx->i2c->addr=0x36;
		aw_dev_err(&i2c->dev, "aw883xx i2c add is from 0x40 to 0x36");
	}
	if(aw883xx->i2c->addr==0x41){
		aw883xx->i2c->addr=0x37;
		aw_dev_err(&i2c->dev, "aw883xx i2c add is from 0x40 to 0x37");
	}
	...
	aw_dev_info(&i2c->dev, "dev_cnt %d probe completed successfully",
		g_aw883xx_dev_cnt);
	set_smartpa_type("aw88394", strlen("aw88394"));
