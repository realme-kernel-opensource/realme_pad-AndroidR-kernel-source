...
static ssize_t aw883xx_re_store(struct device *dev,
					...
					size_t count)
	{
		...
		if (ret < 0)
		 ...
		 aw_dev->cali_desc.cali_re = databuf[0];
	
		return count;
}
	
static ssize_t aw883xx_re_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
		struct aw883xx *aw883xx = dev_get_drvdata(dev);
		struct  aw_device *aw_dev = aw883xx->aw_pa;
...
static ssize_t aw883xx_f0_store(struct device *dev,
					struct device_attribute *attr, const char *buf,
					size_t count)
{
		struct aw883xx *aw883xx = dev_get_drvdata(dev);
		...
		if (ret < 0)
		 ...
		 aw_dev->cali_desc.f0 = databuf[0];
	
 	return count;
}
	
static ssize_t aw883xx_f0_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
		struct aw883xx *aw883xx = dev_get_drvdata(dev);
		struct  aw_device *aw_dev = aw883xx->aw_pa;
...
static DEVICE_ATTR(re, S_IWUSR | S_IRUGO,
			aw883xx_re_show, aw883xx_re_store);
static DEVICE_ATTR(f0, S_IWUSR | S_IRUGO,
			aw883xx_f0_show, aw883xx_f0_store);
