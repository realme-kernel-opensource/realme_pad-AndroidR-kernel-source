...
static struct notifier_block sar_sdslot_notifier;

extern int sd_slot_register_client(struct notifier_block *nb);
extern int sd_slot_unregister_client(struct notifier_block *nb);
						...
						const char *buf, size_t count)
{
	...
	if (state != 0) {
		aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
							(data_en & 0x3f) << 8);
		cal_flag = 1;
        AWLOGE(aw9610x->dev, "awsar calibration ok");
	} else {
		cal_flag = 2;
		AWLOGE(aw9610x->dev, "fail to set aot cali");
	}
...
static DEVICE_ATTR(aot_cali, 0664, sar_docalibration_show, aw9610x_aot_cali_set);
...
static void aw9610x_read_raw_data(struct aw9610x *aw9610x)
{
	...
	pr_err("%s REG_STAT0 = 0x%08x\n", __func__, state0);
	status = (uint8_t)((state0&0x3f000000) >> 24);
	pr_err("%s status = 0x%02x\n", __func__, status);
	value[0] = status & 0x7;
		...
		if(aw9610x->channel_func & (1 << i)){
			...
			if (aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state !=
				...
				case FAR:
					...
					//			ABS_DISTANCE, 0);
					key_val = aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].key_release;
					break;
				case TRIGGER_TH0:
					//input_report_abs(
					//	aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input,
				...
				case TRIGGER_TH3:
					...
					//	aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input,
					//			ABS_DISTANCE, 4);
					key_val = aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].key_press;
					break;
				default:
					AWLOGE(aw9610x->dev, "error abs distance");
					return;
				}
				input_report_key(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input, key_val, 1);
				input_sync(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input);
				input_report_key(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input, key_val, 0);
				input_sync(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input);
			}
			aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].last_state =
					aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state;
	...
	if (gpio_is_valid(aw9610x->irq_gpio)) {
		...
		if (ret) {
			...
			if (ret != 0) {
				...
				enable_irq_wake(aw9610x->to_irq);
				ret = AW_SAR_SUCCESS;
			}
...
static int sar_sdslot_notifier_callback(
			...
			unsigned long event, void *data)
{
	...
	err = aw9610x_i2c_read(aw9610x, REG_SCANCTRL0, &data_en);
	if(err < 0) {
		pr_err("awsar read REG_SCANCTRL0 failed!!");
		return 0;
	}

	aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8), (data_en & 0x3f) << 8);
    pr_err("awsar recieve sd slot changed event and do auto calibration ok");
...
aw9610x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		if(aw9610x->channel_func & (1 << i)){
			...
			__set_bit(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].key_press, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->keybit);
			//__set_bit(KEY_SARCS1, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->keybit);
			//__set_bit(KEY_SARCS2, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->keybit);
			__set_bit(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].key_release, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->keybit);
			//__set_bit(KEY_SAR_RELEASECS1, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->keybit);
			//__set_bit(KEY_SAR_RELEASECS2, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->keybit);
	...
	sar_sdslot_notifier.notifier_call = sar_sdslot_notifier_callback;
	if (sd_slot_register_client(&sar_sdslot_notifier)) {
		pr_err("register sar_sdslot_notifier fail!!!\n");
	}
...
static int32_t aw9610x_i2c_remove(struct i2c_client *i2c)
{
	...
	sysfs_remove_group(&i2c->dev.kobj, &aw9610x_sar_attribute_group);
	sd_slot_unregister_client(&sar_sdslot_notifier);
	return 0;
}
