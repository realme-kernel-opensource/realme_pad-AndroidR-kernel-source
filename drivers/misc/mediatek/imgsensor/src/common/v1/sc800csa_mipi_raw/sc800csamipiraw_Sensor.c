...
#define PFX "sc800cs_main_camera_sensor"
...
#define SC800CSA_SENSOR_GAIN_BASE             1024
#define SC800CSA_SENSOR_GAIN_MAX              65024 //(63.5 * SC800CSA_SENSOR_GAIN_BASE)
#define SC800CSA_SENSOR_GAIN_MAX_VALID_INDEX  6
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	kal_uint16 reg_gain = gain << 4;

	if (reg_gain < SC800CSA_SENSOR_GAIN_BASE)
		reg_gain = SC800CSA_SENSOR_GAIN_BASE;
	else if (reg_gain > SC800CSA_SENSOR_GAIN_MAX)
		reg_gain = SC800CSA_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC800CSA_AGC_Param[SC800CSA_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC800CSA_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC800CSA_AGC_Param[gain_index][0])
			break;
	...
	write_cmos_sensor(0x3e09, SC800CSA_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC800CSA_SENSOR_GAIN_BASE / SC800CSA_AGC_Param[gain_index][0];
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			LOG_INF("sc800csamipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
...
UINT32 SC800CSA_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
