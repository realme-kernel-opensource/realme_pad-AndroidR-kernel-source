...
unsigned int hi846_sub_read_region(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size){
	...
	if(size == HI846_OTP_SIZE) {
	    ...
	    memcpy(buffer_temp, hi846_otp_data.af, HI846_DATA_LEN_AF); // not use
	...
	hi846_dump_otp((kal_uint8 *)&hi846_otp_data, sizeof(hi846_otp_data), 0x847);
