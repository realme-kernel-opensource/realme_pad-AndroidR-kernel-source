...
//add lcd_name node 2021.05.18 start
static ssize_t fb_lcd_name(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	if(islcmconnected){
		if(mtkfb_lcm_name[0] != 0){
			sprintf(buf, "%s panel\n", mtkfb_lcm_name);
		}else{
...
static int lcd_name_create_sysfs(void)
{
	int ret;
...
//add lcd_name node 2021.05.18 end
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	//Begin yanganfa add lcd info name
	lcd_name_create_sysfs();
	//end yanganfa add lcd info
	init_state++;		/* 5 */
