...
extern struct oplus_chg_operations  oplus_chg_sgm41510_ops;
...
/* Jianchao.Shi@BSP.CHG.Basic, 2019/06/10, sjc Modify for charging */
	...
	if (info != NULL) {
		...
		if (pdata->thermal_charging_current_limit == -1) {
				...
				oplus_chg_set_thermal_charging_current(chip);
				chr_err("%s: dev:%s en:%d\n", __func__,
					dev_name(consumer->dev), pdata->thermal_charging_current_limit);
		} else {
			chip->thermal_config = true;
			_mtk_charger_change_current_setting(info);
		}
		_wake_up_charger(info);
		return 0;
...
/*Shouli.Wang@ODM_WT.BSP.CHG 2019/11/12, add for usbtemp*/
#define USB_PORT_PULL_UP_R      100000 //390K
...
int mtk_chgstat_notify(struct oplus_chg_chip *chip)
{
	int ret = 0;
	char *env[2] = { "CHGSTAT=1", NULL };

	chr_err("%s: 0x%x\n", __func__, chip->notify_code);
	ret = kobject_uevent_env(&chip->dev->kobj, KOBJ_CHANGE, env);
	if (ret)
			...
			if(oplus_chip->is_double_charger_support) {
				switch(charger_ic__det_flag) {
					...
					case (1<<SY6974|1<<BQ2589X):
						...
						break;
					case (1<<SGM41510|1<<BQ25601D):
						//oplus_chip->chg_ops = &oplus_chg_sgm41510_ops;
						//oplus_chip->sub_chg_ops = &bq25601d_chg_ops;
						chg_err("charger IC match successful (4)\n");
					break;
				default:
				...
				switch(charger_ic__det_flag) {
					...
					case (1<<SGM41510):
						oplus_chip->chg_ops = &oplus_chg_sgm41510_ops;
						chg_err("charger IC match successful (7)\n");
						break;
					default:
						oplus_chip->chg_ops = &oplus_chg_default_ops;
				}
