...
int z350_qc_mode_select(u8 mode)
{
	int ret = -1;
	...
	ret = z350_write_byte(z350_chip, Z350_REG_QC_MS, mode);
...
int z350_set_apsd_rerun(bool en)
{
	int ret = -1;
	if (!en)
		ret = z350_write_byte(z350_chip, Z350_REG_RERUN_APSD, DISABLE);
	else
		ret = z350_write_byte(z350_chip, Z350_REG_RERUN_APSD, ENABLE);

	if (ret)
		z350_err("%s: set rerun apsd[%d] failed!\n", __func__, en);
	else
		z350_err("%s: set rerun apsd[%d] success!\n", __func__, en);
...
int z350_enable_chgdet_flow(bool en)
{
	...
	z350_err("_z350_enable_chgdet_flow enter! en = %d\n", en);
	z350_chip->enable_chgdet = en;
	if (en)
...
static irqreturn_t z350_interrupt(int irq, void *dev_id)
{
	...
	if (chip->otg_enable || (z350_chip->enable_chgdet == false))
		return IRQ_HANDLED;
...
*/	
	...
	if (pmic_chrdet_status() == false) {
		z350_err("pmic_chrdet_status = false\n");
		...
		charging_poweron = true;
		z350_err("pmic_chrdet_status = true\n");
	}
