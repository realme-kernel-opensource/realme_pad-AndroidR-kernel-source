...
#include "../../supply/mediatek/charger/mtk_intf.h"
...
int wt6670f_qc_mode_select(u8 mode)
{
	int ret = -1;

	if (mode > MODE_QC3_PLUS_V5) {
		wt6670f_err("%s: qc mode[%d] is wrong!\n", __func__, mode);
		return ret;
	}

	ret = wt6670f_write_byte(wt6670f_chip, WT6670F_REG_QC_MS, mode);
...
int wt6670f_get_charger_type(u8 *chg_type)
{
	...
	if (ret) {
		wt6670f_err("get error/state failed!\n");
		*chg_type = 0;
		//wt6670f_chip->charger_online = false;
	} else {
		wt6670f_log("get error/state success, 0x%04x!\n", val);
		val = (u16)(val & 0xFF);
		...
		case QC35_FLOAT:
			*chg_type = SGM41510_CHG_TYPE_FLOAT;
			//wt6670f_chip->charger_online = true;
			break;
		case QC35_SDP:
...
int wt6670f_set_apsd_rerun(bool en)
{
	int ret = -1;
	if (!en)
		ret = wt6670f_write_byte(wt6670f_chip, WT6670F_REG_RERUN_APSD, DISABLE);
	else
		ret = wt6670f_write_byte(wt6670f_chip, WT6670F_REG_RERUN_APSD, ENABLE);

	if (ret)
		wt6670f_err("%s: set rerun apsd[%d] failed!\n", __func__, en);
	else
		wt6670f_err("%s: set rerun apsd[%d] success!\n", __func__, en);
...
static irqreturn_t wt6670f_interrupt(int irq, void *dev_id)
{
	...
	if (chip->otg_enable || (wt6670f_chip->enable_chgdet == false))
		return IRQ_HANDLED;
...
static int load_fw(struct device *dev, const char *fn, bool force)
{
	...
	if (sgm41510_charger_exist_detect() == false) {
		//dev_err(dev, "wt6670f charger_online 0\n");
