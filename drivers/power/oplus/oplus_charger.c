...
enum power_supply_property oplus_batt_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_ENABLED,	/*add for MMI_CHG_TEST*/
#ifdef CONFIG_OPLUS_CHARGER_MTK
...
if (chip == NULL) {
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
			...
			} else if (chip->notify_code && (BATTERY_STATUS__HIGH_TEMP || BATTERY_STATUS__LOW_TEMP)) {
					pr_err("chip->notify_code = %d, tbatt_status = %d\n", chip->notify_code, chip->tbatt_status);
					val->intval = POWER_SUPPLY_STATUS_CHARGING;
...
#endif
			break;
		case POWER_SUPPLY_PROP_CHARGE_ENABLED:	/*add for MMI_CHG TEST*/
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_usbchg_current_ma", &chip->limits.temp_normal_usbchg_current_ma);/* 16C ~ 44C */
		if (rc) {
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
...
#endif
#endif
		if (chip->thermal_config ==true) {
				chg_err(" is thermal set charging current, don't set here\n");
				return;
		}
	...
	if(chip->charger_type == POWER_SUPPLY_TYPE_USB) {
		charging_current = chip->limits.temp_normal_usbchg_current_ma;
		charger_xlog_printk(CHG_LOG_CRTI,
				"[BATTERY]usb detect, charging current: %d\n", charging_current);
		}
...
void oplus_chg_set_thermal_charging_current(struct oplus_chg_chip *chip)
{
	oplus_chg_set_charging_current(chip);
	chg_err(" Go out thermal setting , switch to normal set \n");
}
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
		if (!chip->dual_charger_support || (chip->dual_charger_support && chip->charger_volt > 7500)) {
			...
			} else if (chip->temperature > 340 && chip->temperature <= 370) {
			...
			charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]LED STATUS CHANGED, IS ON\n");
				...
				if (chip->led_temp_status == LED_TEMP_STATUS__HIGH) {
					if (current_limit > chip->limits.input_current_led_ma) {
	...
	chip->chg_ops->set_safe_charging_status();
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.hot_bat_decidegc || batt_temp < chip->limits.cold_bat_decidegc) {
		...
		if (temp_counts >= TEMP_CNT) {
			...
			if (batt_temp < -50 && chip->usb_temp_r >= 0) {	/*-77 remove*/
				tbatt_status = BATTERY_STATUS__REMOVED;
			}
		}
	} else {
...
//	chip->anti_shake_bound.overtemp_bound = chip->limits.overtemp_bat_decidegc;
	...
	chip->pd_chging = false;
	chip->pd_svooc = false;
	chip->first_det_flg = 0;
	chip->thermal_config = false;
}
...
static void oplus_chg_check_rechg_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) {				//4.0
		recharging_vol = oplus_chg_get_float_voltage(chip) - 300;
				...
				|| (chip->tbatt_status == BATTERY_STATUS__COOL_TEMP)
				|| (chip->tbatt_status == BATTERY_STATUS__LITTLE_COOL_TEMP)) {//4.4
		recharging_vol = oplus_chg_get_float_voltage(chip) - 200;
	} else if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP) {//4.4
		recharging_vol = oplus_chg_get_float_voltage(chip) - 50;
	} else {
		...
		recharging_vol = recharging_vol - 100;
	}
	if ((!chip->authenticate) ||(!chip->hmac)) {
...
/* EVEN,gudi@WT.BSP,2021/01/28,modify plugin recognition speed */
#if defined (CONFIG_OPLUS_CHARGER_MTK6768) || defined (CONFIG_OPLUS_CHARGER_MTK6769R) || defined (CONFIG_OPLUS_CHARGER_MTK6769)
			if(chip->charger_type != chip->chg_ops->get_charger_type()) {
				...
				if ((chip->first_det_flg == 2) && (chip->chg_ops->get_charger_type() == POWER_SUPPLY_TYPE_USB_DCP)) {
					chip->first_det_flg = 0;
					oplus_chg_turn_on_charging(chip);
				} else if ((chip->first_det_flg == 1) && (chip->chg_ops->get_charger_type() == POWER_SUPPLY_TYPE_USB_DCP) && chip->pd_chging) {
					chg_err("pd_chging = %d\n", chip->pd_chging);
					chip->first_det_flg = 0;
					oplus_chg_turn_on_charging(chip);
				}
...
static void battery_notify_tbat_check(struct oplus_chg_chip *chip)
{
	...
	if (BATTERY_STATUS__HIGH_TEMP == chip->tbatt_status) {
		//count_high++;
		charger_xlog_printk(CHG_LOG_CRTI,
				"[BATTERY] bat_temp(%d), BATTERY_STATUS__HIGH_TEMP\n",
				chip->temperature);
		if (chip->charger_exist) {
			//count_high = 3;
...
static void battery_notify_usbtemp_check(struct oplus_chg_chip *chip)
{
	static bool usb_temp_over_flag = false;
	if (chip->dischg_flag ) {
		chip->notify_code |= 1 << NOTIFY_USB_TEMP_OVER;
		usb_temp_over_flag = true;
		mtk_chgstat_notify(chip);
		//charger_xlog_printk(CHG_LOG_CRTI,
		//	"[BATTERY] check_usbtemp_status dischg_flag=%d\n", chip->dischg_flag);
	}
	if (usb_temp_over_flag && (chip->dischg_flag == false)) {
		chip->notify_code &= ~(1 << NOTIFY_USB_TEMP_OVER);
		usb_temp_over_flag = false;
		mtk_chgstat_notify(chip);
		charger_xlog_printk(CHG_LOG_CRTI,
			"[BATTERY] check_usbtemp_status dischg_flag=%d\n", chip->dischg_flag);
	}
...
/*charger_xlog_printk(CHG_LOG_CRTI,
			"battery_notify_flag_check, notify_flag = d%, notify_code = %d\n", chip->notify_flag, chip->notify_code);*/
	...
	/*charger_xlog_printk(CHG_LOG_CRTI,
			"battery_notify_flag_check--1, notify_flag = d%, notify_code = %d\n", chip->notify_flag, chip->notify_code);*/
}
...
static void oplus_chg_ibatt_check_and_set(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
		recharge_volt = chip->limits.temp_little_cold_vfloat_mv - 200;
		current_init = chip->limits.temp_little_cold_fastchg_current_ma;
		current_limit = chip->batt_capacity_mah * 15 / 100;
		threshold = 50;
	} else if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP) {
		recharge_volt = chip->limits.temp_warm_vfloat_mv - 50;
		...
		recharge_volt = chip->limits.temp_normal_vfloat_mv- 100;
		...
		recharge_volt = chip->limits.temp_cool_vfloat_mv - 200;
		if (vbatt_higherthan_4180mv) {
		...
		recharge_volt = chip->limits.temp_little_cool_vfloat_mv - 200;
...
static void oplus_chg_update_work(struct work_struct *work)
{
	...
	battery_notify_usbtemp_check(chip);
