...
struct sgm41510_info *sgm41510_info;
bool is_hvdcp_enabled = false;
static int sgm41510_driver_probe(struct i2c_client *client,
				const struct i2c_device_id *id);
static int sgm41510_set_watch_dog_timer(struct sgm41510_info *info, u32 sec);
/*
...
static int sgm41510_dump_register(struct charger_device *chg_dev)
{
	...
	pr_info("sgm41510_dump_register [sgm41510] ");
	...
	for (i = 0; i < sgm41510_REG_NUM; i++) {
		...
		pr_info("sgm41510_dump_register [0x%x]=0x%x ", i, sgm41510_reg[i]);
	}
	pr_debug("\n");
	if (qc_module_z350)
		z350_dump_register();
...
{
	int status = 0;
	struct sgm41510_info *info = dev_get_drvdata(&chg_dev->dev);
...
static int sgm41510_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (en) {
		...
		sgm41510_set_watch_dog_timer(info, 160);
		gpio_set_value(info->sgm41510_otgen_gpio, 1);
	} else {
...
static int sgm41510_parse_dt(struct sgm41510_info *info,
			    struct device *dev)
{
...
static int sgm41510_inform_psy_changed(struct sgm41510_info *info)
{
	int ret = 0;
...
static inline int __sgm41510_enable_chgdet_flow(struct sgm41510_info *info, bool en);
static int __sgm41510_chgdet_handler(struct sgm41510_info *info)
{
	int ret = 0;
...
static int sgm41510_chgdet_handler(struct sgm41510_info *info)
{
	int ret = 0;
...
int sgm41510_attachi_irq_handler(void)
{
	...
	dev_notice(sgm41510_info->dev, "%s\n", __func__);

	// check bc12_en state
	mutex_lock(&sgm41510_info->bc12_access_lock);
	if (!sgm41510_info->bc12_en) {
		dev_notice(sgm41510_info->dev, "%s: bc12 disabled, ignore irq\n",
			__func__);
		goto out;
	}
	ret = __sgm41510_chgdet_handler(sgm41510_info);
out:
	mutex_unlock(&sgm41510_info->bc12_access_lock);
	return ret;
}

static int sgm41510_set_usbsw_state(struct sgm41510_info *info, int state)
{
...
static int sgm41510_enable_chgdet_flow(struct sgm41510_info *info, bool en)
{
	int ret = 0;//, i = 0;
	//bool pwr_rdy = false;
...
/*
	if (en) {
		...
		for (i = 0; i < max_wait_cnt; i++) {
			...
			ret = sgm41510_i2c_test_bit(info, RT9467_REG_CHG_STATC,
				RT9467_SHIFT_PWR_RDY, &pwr_rdy);
			if (ret >= 0 && !pwr_rdy) {
...
{
	int ret = 0;
	struct sgm41510_info *info = NULL;

	pr_info("[%s]\n", __func__);

	info = devm_kzalloc(&client->dev, sizeof(struct sgm41510_info),
			    GFP_KERNEL);
	if (!info)
		return -ENOMEM;
...
static int __init sgm41510_init(void)
{
	...
	ret = platform_device_register(&sgm41510_user_space_device);
	if (ret) {
