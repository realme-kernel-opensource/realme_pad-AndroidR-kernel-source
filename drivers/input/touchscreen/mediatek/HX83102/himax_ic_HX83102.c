...
void hx83102_burst_enable(uint8_t auto_add_4_byte)
{
	...
	ret = himax_bus_write(0x13, tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	if (ret < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
	...
	ret = himax_bus_write(0x0D, tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	if (ret < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
...
int hx83102_flash_write_burst(uint8_t *reg_byte, uint8_t *write_data)
{
	...
	ret = himax_bus_write(0x00, data_byte, 8, HIMAX_I2C_RETRY_TIMES);
	if (ret < 0) {
		E("%s: i2c access fail!\n", __func__);
		return I2C_FAIL;
	}
		...
		uint8_t *read_data)
{
	...
	ret = himax_bus_read(0x08, read_data, read_length,
		HIMAX_I2C_RETRY_TIMES);
	if (ret < 0) {
		E("%s: i2c access fail!\n", __func__);
		return I2C_FAIL;
...
static void hx83102ab_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		...
		g_core_fp.fp_register_read(pfw_op->addr_smwp_enable,
...
static int hx83102d_0f_excp_check(void)
{
	...
	g_core_fp.fp_register_read((pzf_op)->addr_sts_chk,
			DATA_LEN_4, tmp_data, 0);
	...
	g_core_fp.fp_register_read((pzf_op)->addr_activ_relod,
			DATA_LEN_4, tmp_data, 0);
 ...
 */
	...
	count = 0;
	do {
		g_core_fp.fp_register_read(handshaking_addr, DATA_LEN_4,
				recv_data, 0);
	...
	if (count < 10) {
		...
		do {
			g_core_fp.fp_register_write(handshaking_addr,
					...
					send_data, 0);
			usleep_range(1000, 1100);
			g_core_fp.fp_register_read(handshaking_addr,
					DATA_LEN_4,
					recv_data, 0);
...
static bool hx83102_chip_detect(void)
{
	...
	ret = himax_bus_read(0x13, tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	if (ret < 0) {
		E("%s: i2c access fail!\n", __func__);
		return false;
	}
