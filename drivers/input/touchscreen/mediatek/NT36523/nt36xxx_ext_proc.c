...
/*******************************************************
Description:
	touchscreen infomation show function.
...
{
	seq_printf(m, "[Vendor]%s, [IC]NT36523, fw_ver=%d, x_num=%d, y_num=%d, button_num=%d\n", "DJN", ts->fw_ver, ts->x_num, ts->y_num, ts->max_button_num);
	return 0;
}
...
/*******************************************************
Description:
	touchscreen /proc/tp_info open
	function.

return:
	n.a.
*******************************************************/
static int32_t tp_info_open(struct inode *inode, struct file *file)
{
	if (mutex_lock_interruptible(&ts->lock)) {
		return -ERESTARTSYS;
	}
	...
	return seq_open(file, &tp_info_seq_ops);
}
...
{
	NVT_proc_fw_version_entry = proc_create(TP_INFO, 0444, NULL, &tp_info_fops);
