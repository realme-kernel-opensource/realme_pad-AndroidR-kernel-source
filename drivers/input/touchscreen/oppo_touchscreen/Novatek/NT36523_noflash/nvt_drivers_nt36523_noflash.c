 ...
 * VENDOR_EDIT
 * File       : novatek_drivers_nt36523.c
 * Description: Source file for novatek nt36523 driver
 * Version   : 1.0
 * Date        : 2019/019/22
...
void nvt_sw_reset_idle(struct chip_data_nt36523 *chip_info)
{
...
static void nvt_auto_test(struct seq_file *s, void *chip_data, struct nvt_testdata *nvt_testdata)
{
	...
	//offset
	ph->array_fw_rawdata_P_offset = 0xcc;
	ph->array_fw_rawdata_N_offset = 0x174c;
	ph->array_open_rawdata_P_offset = 0x2dcc;
	ph->array_open_rawdata_N_offset = 0x444c;
	ph->array_Short_Rawdata_P_offset = 0x5acc;
	ph->array_Short_Rawdata_N_offset = 0x714c;
	ph->array_FW_CC_P_offset = 0x87cc;
	ph->array_FW_CC_N_offset = 0x9e4c;
	ph->array_FW_Diff_P_offset = 0xb4cc;
	ph->array_FW_Diff_N_offset = 0xcb4c;
#ifdef CONFIG_TOUCHPANEL_NT_DIGITALNOISE_TEST
	ph->array_FW_Digital_Diff_P_offset = 0xe1cc;
	ph->array_FW_Digital_Diff_N_offset = 0xf84c;
#endif
	ph->array_Doze_Diff_P_offset = 0x10ecc;
	ph->array_Doze_Diff_N_offset = 0x1104c;
	ph->array_Doze_Rawdata_P_offset = 0x111cc;
	ph->array_Doze_Rawdata_N_offset = 0x1134c;
							...
							size_t len, loff_t *pos)
{
	...
	if (nvt_get_fw_info_noflash(g_chip_info)) {
        TPD_INFO("tp_info node get fw info failed!\n");
	}

	ret += snprintf(buf_tmp, sizeof(buf_tmp),
				"[VENDOR] = DJN, [IC] = NT36523B, [FW_VER] = 0x%02X\n", g_chip_info->fw_ver);
...
static int nvt_tp_probe(struct spi_device *client)
{
	...
	struct proc_dir_entry *tp_info_file;
    int ret = -1;
	...
	tp_info_file = proc_create("tp_info", 0777,
			NULL, &tp_info_ops);
	if (tp_info_file == NULL) {
		TPD_INFO(" %s: proc vendor file create failed!\n", __func__);
		goto err_tp_info;
	}
