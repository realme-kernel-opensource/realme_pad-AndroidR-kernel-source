...
static void tp_gesture_handle(struct touchpanel_data *ts)
{
	...
	TPD_INFO("gesture_info_temp.gesture_type = %d", gesture_info_temp.gesture_type);
...
void global_tp_suspend(void)
{
	if ((g_tp != NULL) && (g_tp->dev != NULL)) {
		tp_suspend(g_tp->dev);
	}
}
...
static void speedup_resume(struct work_struct *work)
{
    ...
    if (!ts->skip_reset_in_resume && !ts->fp_info.touch_state) {
		TPD_INFO("ts->lcd_trigger_load_tp_fw_support = %d", ts->lcd_trigger_load_tp_fw_support);
        if (!ts->lcd_trigger_load_tp_fw_support) {
            ts->ts_ops->reset(ts->chip_data);
        }
    }
...
{
	TPD_INFO("CONFIG_TOUCHPANEL suspend!\n");
    ts->i2c_ready = false;
	TPD_INFO("ts->black_gesture_support = %d\n", ts->black_gesture_support);
    if (ts->black_gesture_support || ts->fingerprint_underscreen_support) {
        if ((ts->gesture_enable & 0x01) == 1 || ts->fp_enable) {
            /*enable gpio wake system through interrupt*/
            enable_irq_wake(ts->irq);
			TPD_INFO("CONFIG_TOUCHPANEL_MTK_PLATFORM irq wake support!\n");
            return;
        }
    }
    disable_irq(ts->irq);
...
void tp_i2c_suspend(struct touchpanel_data *ts)
{
    ...
    if (ts->black_gesture_support || ts->fingerprint_underscreen_support) {
        if ((ts->gesture_enable & 0x01) == 1 || ts->fp_enable) {
            /*enable gpio wake system through interrupt*/
            enable_irq_wake(ts->irq);
			TPD_INFO("suspend! ts->irq = %d\n", ts->irq);
            if (ts->new_set_irq_wake_support) {
				TPD_INFO("irq wake support!\n");
                return;
            }
        }
    }
