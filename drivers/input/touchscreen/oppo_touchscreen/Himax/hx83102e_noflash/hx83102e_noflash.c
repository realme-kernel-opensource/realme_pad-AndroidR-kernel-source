 ...
 * VENDOR_EDIT
 * File       : hx83102e_drivers_s4322.c
 * Description: Source file for hx83102e S4322 driver
 * Version   : 1.0
 * Date        : 2017-04-17
...
#define TPD_DEVICE "himax,hx83102e_nf"
...
struct chip_data_hx83102e *g_chip_info;
...
char *g_rslt_data;
int **hx83102e_nf_inspection_criteria;
int *hx83102e_nf_inspt_crtra_flag;
...
int hx83102e_nf_fail_write_count;

#ifdef HX_ZERO_FLASH
extern int g_f_0f_updat;
/* 128k+ */
int hx83102e_nf_cfg_crc = -1;
int hx83102e_nf_cfg_sz;
uint8_t hx83102e_nf_sram_min[4];
unsigned char *hx83102e_nf_FW_buf;
/* 128k- */
...
static int hx83102e_power_control(void *chip_data, bool enable);
static int hx83102e_get_chip_info(void *chip_data);
static int hx83102e_mode_switch(void *chip_data, work_mode mode, bool flag);
static uint32_t himax_hw_check_CRC(uint8_t *start_addr, int reload_length);
static fw_check_state hx83102e_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data);
static void himax_read_FW_ver(void);
#ifdef HX_RST_PIN_FUNC
static int hx83102e_resetgpio_set(struct hw_resource *hw_res, bool on);
#endif
...
#ifdef HX_RST_PIN_FUNC
            hx83102e_resetgpio_set(g_chip_info->hw_res, false); // reset gpio
            hx83102e_resetgpio_set(g_chip_info->hw_res, true); // reset gpio
#else
            himax_mcu_sys_reset();
#endif
        }
...
static void himax_hx83102e_reload_to_active(void)
{
	uint8_t addr[4] = {0};
	uint8_t data[4] = {0};
	uint8_t retry_cnt = 0;
...
void himax_sense_on_by_sys_rst(void)
{
	...
	himax_hx83102e_reload_to_active();
...
/**
 * hx83102e_enable_interrupt -   Device interrupt ability control.
 * @chip_info: struct include i2c resource.
 * @enable: disable or enable control purpose.
 * Return  0: succeed, -1: failed.
 */

static int hx83102e_enable_interrupt(struct chip_data_hx83102e *chip_info, bool enable)
{
...
int himax_sram_write_crc_check(const struct firmware *fw_entry, uint8_t *addr, int strt_idx, uint32_t len)
{
    ...
    TPD_INFO("%s, addr=0x%02X%02X%02X%02X, start_idx=%d=0x%08X, len=%d\n", 
...
static int hx83102e_0f_overlay(int ovl_type, int mode)
{
	uint8_t count = 0;
	uint8_t count2 = 0;
	uint8_t part_num = 0;
	...
	/*1. get number of partition*/
            TPD_INFO("%s, cfg_table_pos=0x%08X\n", __func__, cfg_table_pos);
        ...
        hx83102e_enable_interrupt(g_chip_info, false);
...
*		himax_sense_on(0x00);
*		hx83102e_enable_interrupt(g_chip_info, true);
*	}
*/
		ret = 0;
...
static bool hx_bin_desc_data_get(uint32_t addr, uint8_t *flash_buf)
{
	...
	for (i = 0; i < FW_PAGE_SZ; i = i + data_sz) {
		...
		if (!chk_end) { /*1. Check all zero*/
			...
			TPD_INFO("%s: chk sum failed in %X\n",	__func__, i + addr);
		} else { /*3. get data*/
...
bool hx_parse_bin_cfg_data(const struct firmware *fw_entry)
{
    ...
    for (i = 0; i < 4; i++)
        hx83102e_nf_sram_min[i] = g_zf_info_arr[i_min].sram_addr[i];
    hx83102e_nf_cfg_sz = (dsram_max - dsram_base) + g_zf_info_arr[i_max].write_size;
    // hx83102e_nf_cfg_sz = hx83102e_nf_cfg_sz + (hx83102e_nf_cfg_sz % 16);
    if (hx83102e_nf_cfg_sz % 4 > 0)
        hx83102e_nf_cfg_sz = hx83102e_nf_cfg_sz + (hx83102e_nf_cfg_sz % 4);


    TPD_INFO("%s, hx83102e_nf_cfg_sz = %d!, dsram_base = %X, dsram_max = %X\n", __func__, hx83102e_nf_cfg_sz, dsram_base, dsram_max);

    if (hx83102e_nf_FW_buf == NULL)
        hx83102e_nf_FW_buf = kzalloc(sizeof(unsigned char) * FW_BIN_16K_SZ, GFP_KERNEL);
...
#endif
        memcpy(hx83102e_nf_FW_buf + (g_zf_info_arr[i].cfg_addr - dsram_base), (unsigned char *)&fw_entry->data[g_zf_info_arr[i].fw_addr], g_zf_info_arr[i].write_size);
    }
    hx83102e_nf_cfg_crc = himax_mcu_Calculate_CRC_with_AP(hx83102e_nf_FW_buf, 0, hx83102e_nf_cfg_sz);
    TPD_INFO("hx83102e_nf_cfg_crc = %d\n", hx83102e_nf_cfg_crc);
    return true;
}

static int hx83102e_nf_zf_part_info(const struct firmware *fw_entry)
{
    bool ret = false;
    bool flag_1k_header = false;
    ...
    do {
        himax_mcu_register_write(hx83102e_nf_sram_min, hx83102e_nf_cfg_sz, hx83102e_nf_FW_buf, 0);
        crc = himax_hw_check_CRC(hx83102e_nf_sram_min, hx83102e_nf_cfg_sz);
        if (crc != hx83102e_nf_cfg_crc)
            TPD_INFO("Config CRC FAIL, HW CRC = %X, SW CRC = %X, retry time = %d", crc, hx83102e_nf_cfg_crc, retry);
        retry++;
    } while (!ret && retry < 10);
...
//#if defined(HX_SMART_WAKEUP)
	...
	//} else if (private_ts->is_suspended && private_ts->gesture_enable) {
	//	ovl_idx_t = ovl_idx[1];
...
fw_reload:
    ...
    if ((int)tmp_fw_entry->size > HX64K) {
        ret = hx83102e_nf_zf_part_info(tmp_fw_entry);
    } else {
        g_core_fp.fp_sys_reset();
        himax_sense_off();
        /* first 64K */
    ...
    hx83102e_fw_check(private_ts->chip_data, &private_ts->resolution_info, &private_ts->panel_data);
...
bool himax_ic_package_check(void)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10) && (tmp_data[1] == 0x2e)) {
            IC_TYPE = HX_83102E_SERIES_PWON;
            IC_CHECKSUM = HX_TP_BIN_CHECKSUM_CRC;
            hx_0f_init();
            //Himax: Set FW and CFG Flash Address
            FW_VER_MAJ_FLASH_ADDR   = 59397;  //0x00C005
            FW_VER_MAJ_FLASH_LENG   = 1;
            FW_VER_MIN_FLASH_ADDR   = 59398;  //0x00C006
            FW_VER_MIN_FLASH_LENG   = 1;
            CFG_VER_MAJ_FLASH_ADDR = 59648;  //0x00C100
            CFG_VER_MAJ_FLASH_LENG = 1;
            CFG_VER_MIN_FLASH_ADDR = 59649;  //0x00C101
            CFG_VER_MIN_FLASH_LENG = 1;
            CID_VER_MAJ_FLASH_ADDR = 59394;  //0x00C002
            CID_VER_MAJ_FLASH_LENG = 1;
            CID_VER_MIN_FLASH_ADDR = 59395;  //0x00C003
...
#endif
            TPD_INFO("Himax IC package 83102e_in\n");
            ret_data = true;
            break;
        } else {
            ret_data = false;
...
void himax_read_OPPO_FW_ver(struct chip_data_hx83102e *chip_info)
{
    uint8_t cmd[4];
    uint8_t data[4];
...
/*Himax DB Start*/
#if 0
static int hx83102e_lcd_resetgpio_set(struct hw_resource *hw_res, bool on)
{
    int ret = 0;
...
void himax_esd_hw_reset(struct chip_data_hx83102e *chip_info)
{
    int ret = 0;
    ...
    do {
    #ifdef HX_RST_PIN_FUNC
        hx83102e_resetgpio_set(chip_info->hw_res, true); // reset gpio
        hx83102e_resetgpio_set(chip_info->hw_res, false); // reset gpio
        hx83102e_resetgpio_set(chip_info->hw_res, true); // reset gpio
    #else
        g_core_fp.fp_sys_reset();
    #endif
...
int himax_checksum_cal(struct chip_data_hx83102e *chip_info, uint8_t *buf, int ts_status)
{
...
int hx_test_data_pop_out(struct chip_data_hx83102e *chip_info, char *g_Test_list_log, char *g_Company_info_log, char *g_project_test_info_log, char *rslt_buf, char *filepath)
{
...
int hx_test_data_get(struct chip_data_hx83102e *chip_info, uint32_t RAW[], char *start_log, char *result, int now_item)
{
    uint32_t i;
...
int himax_get_rawdata(struct chip_data_hx83102e *chip_info, uint32_t *RAW, uint32_t datalen)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t *tmp_rawdata;
...
int mpTestFunc(struct chip_data_hx83102e *chip_info, uint8_t checktype, uint32_t datalen)
{
    uint8_t tmp_addr[4] = {0};
    uint8_t tmp_data[4] = {0};
    ...
    case HIMAX_INSPECTION_OPEN:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_OPENMAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_OPENMIN][i]) {
                    TPD_INFO("%s: open test FAIL\n", __func__);
                    ret = RESULT_ERR;
                }
            }
    ...
    case HIMAX_INSPECTION_MICRO_OPEN:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_M_OPENMAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_M_OPENMIN][i]) {
                    TPD_INFO("%s: micro open test FAIL\n", __func__);
                    ret =  RESULT_ERR;
                }
            }
        ...
        TPD_INFO("M_OPENMAX = %d, M_OPENMIN = %d\n",
                 hx83102e_nf_inspection_criteria[IDX_M_OPENMAX][1],
                 hx83102e_nf_inspection_criteria[IDX_M_OPENMIN][1]);
    ...
    case HIMAX_INSPECTION_SHORT:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_SHORTMAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_SHORTMIN][i]) {
                    TPD_INFO("%s: short test FAIL\n", __func__);
                    ret = RESULT_ERR;
                }
            }
    ...
    case HIMAX_INSPECTION_RAWDATA:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_RAWDATA_MAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_RAWDATA_MIN][i]) {
                    TPD_INFO("%s: rawdata test FAIL\n", __func__);
                    ret = RESULT_ERR;
                }
            }
    ...
    case HIMAX_INSPECTION_LPWUG_RAWDATA:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_LPWUG_RAWDATA_MAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_LPWUG_RAWDATA_MIN][i]) {
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_RAWDATA FAIL\n", __func__);
                    ret = THP_AFE_INSPECT_ERAW;
                }
            }
        ...
        if(isBD12proj || isEA006proj) {
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if(isRead_csv == false) {
                    ...
                    if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_LPWUG_NOISE_MAX][i] ||
                        RAW[i] < hx83102e_nf_inspection_criteria[IDX_LPWUG_NOISE_MIN][i]) {
                        TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n", __func__);
                        ret = THP_AFE_INSPECT_ENOISE;
                    }
                }
            ...
            if(isRead_csv == false) {
                ...
                if (weight > LPWUG_NOISEMAX || weight < hx83102e_nf_inspection_criteria[IDX_LPWUG_NOISE_MIN][i]) {
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n", __func__);
                    ret = THP_AFE_INSPECT_ENOISE;
                }
            }
    ...
    case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_LPWUG_IDLE_RAWDATA_MAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_LPWUG_IDLE_RAWDATA_MIN][i]) {
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA FAIL\n", __func__);
                    ret = THP_AFE_INSPECT_ERAW;
                }
            }
    ...
    case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83102e_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MAX][i] ||
                    RAW[i] < hx83102e_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MIN][i]) {
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE FAIL\n", __func__);
                    ret = THP_AFE_INSPECT_ENOISE;
                }
            }
...
int hx_print_crtra_after_parsing(struct chip_data_hx83102e *chip_info)
{
    int i = 0, j = 0;
    ...
    for (i = 0; i < HX_CRITERIA_SIZE; i++) {
        ...
        if (hx83102e_nf_inspt_crtra_flag[i] == 1) {
            for (j = 0; j < all_mut_len; j++) {
                TPD_DEBUG("%d, ", hx83102e_nf_inspection_criteria[i][j]);
                if (j % 16 == 15)
                    TPD_DEBUG("\n");
            }
        } else {
...
static int hx_get_crtra_by_name(struct chip_data_hx83102e *chip_info, char **result, int size_of_result_str)
{
    int i = 0;
    ...
    for (i = 0; i < size_of_result_str && result[i] != NULL; i++) {
        ...
        if (i == 0 || i == ((i / (all_mut_len)) + (i / (all_mut_len) * (all_mut_len)))) {
            ...
            */
            ...
            if (count_type < 0) {
                ...
                hx83102e_nf_inspt_crtra_flag[count_type] = 1;
            }
            continue;
        }
        ...
        if (temp != -9487) {
            hx83102e_nf_inspection_criteria[count_type][count_data] = temp;
        } else {
        ...
        /* dbg
         * I("[%d]hx83102e_nf_inspection_criteria[%d][%d]=%d\n",
         * i, count_type, count_data,
         * hx83102e_nf_inspection_criteria[count_type][count_data]);
         */
        count_data++;
...
static int himax_parse_criteria_file(struct chip_data_hx83102e *chip_info)
{
    int err = THP_AFE_INSPECT_OK;
...
static int himax_self_test_data_init(struct chip_data_hx83102e *chip_info)
{
    ...
    /* init criteria data*/
    hx83102e_nf_inspt_crtra_flag = kzalloc(HX_CRITERIA_SIZE * sizeof(int), GFP_KERNEL);
    hx83102e_nf_inspection_criteria = kzalloc(sizeof(int *)*HX_CRITERIA_SIZE, GFP_KERNEL);
    if (hx83102e_nf_inspt_crtra_flag == NULL || hx83102e_nf_inspection_criteria == NULL) {
        TPD_INFO("%s: %d, Memory allocation falied!\n", __func__, __LINE__);
        return MEM_ALLOC_FAIL;
    }

    for (i = 0; i < HX_CRITERIA_SIZE; i++) {
        hx83102e_nf_inspection_criteria[i] = kcalloc(
                (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM), sizeof(int), GFP_KERNEL);
        if (hx83102e_nf_inspection_criteria[i] == NULL) {
            TPD_INFO("%s: %d, Memory allocation falied!\n", __func__, __LINE__);
            return MEM_ALLOC_FAIL;
        }
    }
    ...
    for (i = 0 ; i < HX_CRITERIA_SIZE ; i++) {
        if (hx83102e_nf_inspt_crtra_flag[i] != 0)
...
static void hx83102e_black_screen_test(void *chip_data, char *message)
{
    int error = 0;
    int error_num = 0;
    int retry_cnt = 3;
    ...
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
    int i = 0;

    TPD_INFO("%s\n", __func__);

	TPD_INFO("nothing to do!");
	return;
    ...
    hx83102e_nf_fail_write_count = 0;
    ...
    /* return to safe mode*/
    hx83102e_enable_interrupt(chip_info, false);
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 15, "test Item:\n");
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "6. MP_LPWUG_TEST_RAW: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
 //   sprintf(message, "%s\n", buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "7. MP_LPWUG_TEST_NOISE: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
 //   sprintf(message, "%s\n", buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "8. MP_LPWUG_IDLE_TEST_RAW: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
//    sprintf(message, "%s\n", buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "9. MP_LPWUG_IDLE_TEST_NOISE: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
 //   sprintf(message, "%s\n", buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 22, "Final_result: %s\n", error_num ? "Fail" : "Pass");
    if (error) {
...
RET_OUT:
    IN_SELF_TEST = 0;
    if (hx83102e_nf_inspection_criteria != NULL) {
        for (i = 0; i < HX_CRITERIA_SIZE; i++) {
            if (hx83102e_nf_inspection_criteria[i] != NULL) {
                kfree(hx83102e_nf_inspection_criteria[i]);
                hx83102e_nf_inspection_criteria[i] = NULL;
            }
        }
        kfree(hx83102e_nf_inspection_criteria);
        hx83102e_nf_inspection_criteria = NULL;
        TPD_INFO("Now it have free the hx83102e_nf_inspection_criteria!\n");
    } else {
        TPD_INFO("No Need to free hx83102e_nf_inspection_criteria!\n");
    }

    if (hx83102e_nf_inspt_crtra_flag) {
        kfree(hx83102e_nf_inspt_crtra_flag);
        hx83102e_nf_inspt_crtra_flag = NULL;
    }
    /*
    if (g_rslt_data) {
        kfree(g_rslt_data);
    ...
    */
    ...
    //hx83102e_enable_interrupt(chip_info, true); 
}

int himax_chip_self_test(struct seq_file *s, struct chip_data_hx83102e *chip_info, char *g_Test_list_log)
{
    int error = 0;
    int error_num = 0;
    char *buf;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "1. Open Test: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
   // seq_printf(s, buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "2. Micro Open Test: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
  //  seq_printf(s, buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "3. Short Test: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
  //  seq_printf(s, buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "4. Raw data Test: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
  //  seq_printf(s, buf);
    if (error != 0)
        error_num++;
    ...
    hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 32, "5. Noise Test: %s\n", error ? "NG" : "Ok");
    TPD_INFO("%s", buf);
 //   seq_printf(s, buf);
    if (error != 0)
        error_num++;
...
void himax_chip_erase(void)
{
    ...
    if (!wait_wip(100)) {
        TPD_INFO("%s:83102e_Chip_Erase Fail\n", __func__);
    }
...
void himax_flash_programming(uint8_t *FW_content, int FW_Size)
{
    ...
    for (page_prog_start = 0; page_prog_start < FW_Size; page_prog_start = page_prog_start + 256) {
        ...
        if (!wait_wip(1)) {
            TPD_INFO("%s:83102e_Flash_Programming Fail\n", __func__);
        }
    }
RET_OUT:
    if(buring_data)
...
static size_t hx83102e_proc_register_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    size_t ret = 0;
    uint16_t loop_i;
    int i = 0;
...
static size_t hx83102e_proc_register_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    char buf[81] = {0};
    char buf_tmp[16];
    uint8_t length = 0;
...
void himax_ts_diag_func(struct chip_data_hx83102e *chip_info, int32_t *mutual_data)
{
    int i = 0;
    int j = 0;
...
static size_t hx83102e_proc_diag_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    char messages[80] = {0};
    ...
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)ts->chip_data;
...
static size_t hx83102e_proc_diag_read(struct seq_file *s, void *v)
{
    size_t ret = 0;
    uint16_t mutual_num;
    uint16_t self_num;
...
uint8_t himax_read_DD_status(struct chip_data_hx83102e *chip_info, uint8_t *cmd_set, uint8_t *tmp_data)
{
    int cnt = 0;
...
static size_t hx83102e_proc_DD_debug_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    int ret = 0;
    uint8_t tmp_data[64];
    uint8_t loop_i = 0;
...
static size_t hx83102e_proc_DD_debug_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    uint8_t i = 0;
    uint8_t cnt = 2;
    unsigned long result = 0;
...
int himax_read_FW_status(struct chip_data_hx83102e *chip_info, uint8_t *state_addr, uint8_t *tmp_addr)
{
    uint8_t req_size = 0;
...
static size_t hx83102e_proc_FW_debug_read(struct file *file, char *buf,
...
static int hx83102e_configuration_init(struct chip_data_hx83102e *chip_info, bool config)
{
    int ret = 0;
...
int himax_ic_reset(struct chip_data_hx83102e *chip_info, uint8_t loadconfig, uint8_t int_off)
{
    int ret = 0;
    ...
    if (chip_info->hw_res->reset_gpio) {
        if (int_off) {
            ...
            ret = hx83102e_enable_interrupt(chip_info, false);
            if (ret < 0) {
                TPD_INFO("%s: hx83102e enable interrupt failed.\n", __func__);
                return ret;
            }
        }
    #ifdef HX_RST_PIN_FUNC
    ...
    #endif
        if (loadconfig) {
            ret = hx83102e_configuration_init(chip_info, false);
            if (ret < 0) {
                TPD_INFO("%s: hx83102e configuration init failed.\n", __func__);
                return ret;
            }
            ret = hx83102e_configuration_init(chip_info, true);
            if (ret < 0) {
                TPD_INFO("%s: hx83102e configuration init failed.\n", __func__);
                return ret;
            }
        ...
        if (int_off) {
            ret = hx83102e_enable_interrupt(chip_info, true);
            if (ret < 0) {
                TPD_INFO("%s: hx83102e enable interrupt failed.\n", __func__);
                return ret;
            }
...
static size_t hx83102e_proc_reset_write(struct file *file, const char *buff,
                                        size_t len, loff_t *pos)
{
    char buf_tmp[12];
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static size_t hx83102e_proc_sense_on_off_write(struct file *file, const char *buff,
        size_t len, loff_t *pos)
{
    char buf[80] = {0};
    //struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static void himax_debug_sta_judge(struct chip_data_hx83102e *chip_info)
{
...
static int hx83102e_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
...
static int hx83102e_ftm_process(void *chip_data)
{
#ifdef HX_RST_PIN_FUNC
    hx83102e_resetgpio_set(g_chip_info->hw_res, false); // reset gpio
#endif
...
static int hx83102e_get_vendor(void *chip_data, struct panel_info *panel_data)
{
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
...
/**
 * hx83102e_get_fw_id -   get device fw id.
 * @chip_info: struct include i2c resource.
 * Return fw version result.
 */
static uint32_t hx83102e_get_fw_id(struct chip_data_hx83102e *chip_info)
{
...
static fw_check_state hx83102e_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
    ...
    panel_data->TP_FW = hx83102e_get_fw_id(chip_info);
...
static u8 hx83102e_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
...
/*
static int hx83102e_reset_for_prepare(void *chip_data)
{
    int ret = -1;
...
*/
/*
static void hx83102e_resume_prepare(void *chip_data)
{
    //hx83102e_reset_for_prepare(chip_data);
    #ifdef HX_ZERO_FLASH
...
*/
static void hx83102e_exit_esd_mode(void *chip_data)
{
 ...
 */
static int hx83102e_reset(void *chip_data)
{
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
    int ret = 0;
    int load_fw_times = 10;
    ...
    /*Yulianghan@RM.PSW.BSP.TP, 2018/09/07, add for hx83102e_noflash lcd esd TP irq exception in realme 18612*/
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
    enable_irq(chip_info->hx_irq);
#endif
#ifdef HX_ZERO_FLASH	
...
static int hx83102e_enable_black_gesture(struct chip_data_hx83102e *chip_info, bool enable)
{
    int ret = 0;
...
/*
static int hx83102e_enable_edge_limit(struct chip_data_hx83102e *chip_info, bool enable)
{
    int ret = 0;
    return ret;
}
...
static int hx83102e_enable_charge_mode(struct chip_data_hx83102e *chip_info, bool enable)
{
    int ret = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
/*on = 1:on   0:off */
static int hx83102e_jitter_switch (struct chip_data_hx83102e *chip_info, bool on)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
static int hx83102e_enable_headset_mode(struct chip_data_hx83102e *chip_info, bool enable)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
static int hx83102e_rotative_switch(struct chip_data_hx83102e *chip_info, int mode)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
static int hx83102e_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    case MODE_GESTURE:
        ret = hx83102e_enable_black_gesture(chip_info, flag);
        if (ret < 0) {
            TPD_INFO("%s: hx83102e enable gesture failed.\n", __func__);
            return ret;
        }
    ...
    case MODE_EDGE:
        //ret = hx83102e_enable_edge_limit(chip_info, flag);
        ret = hx83102e_rotative_switch(chip_info, flag);
        if (ret < 0) {
            TPD_INFO("%s: hx83102e enable edg & corner limit failed.\n", __func__);
            return ret;
        }
    ...
    case MODE_CHARGE:
        ret = hx83102e_enable_charge_mode(chip_info, flag);
        if (ret < 0) {
    ...
    case MODE_HEADSET:
        ret = hx83102e_enable_headset_mode(chip_info, flag);
        if (ret < 0) {
    ...
    case MODE_GAME:
        ret = hx83102e_jitter_switch(chip_info, !flag);
        if (ret < 0) {
...
static int hx83102e_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    int i = 0;
    int gesture_sign = 0;
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
...
static int hx83102e_power_control(void *chip_data, bool enable)
{
    ...
    if (true == enable) {
        ...
        if (ret)
            return -1;
/*Himax DB Start*/
        //ret = hx83102e_lcd_resetgpio_set(chip_info->hw_res, true);
        //if (ret)
        //    return -1;
/*Himax DB End*/
    #ifdef HX_RST_PIN_FUNC
        ret = hx83102e_resetgpio_set(chip_info->hw_res, true);
        if (ret)
            return -1;
    #endif
    } else {
    #ifdef HX_RST_PIN_FUNC
        ret = hx83102e_resetgpio_set(chip_info->hw_res, false);
        if (ret)
            return -1;
    #endif
...
*/
static int hx83102e_int_pin_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata, char *g_Test_list_log)
{
    ...
    if (eint_count == 10) {
        ...
        hx83102e_nf_fail_write_count += snprintf(g_Test_list_log + hx83102e_nf_fail_write_count, 45, "eint_status is low, TP EINT direct stort, \n");
...
static void hx83102e_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    int error_count = 0;
    int ret = THP_AFE_INSPECT_OK;
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
    ...
    error_count += hx83102e_int_pin_test(s, chip_info, syna_testdata, g_Test_list_log);
...
static void hx83102e_read_debug_data(struct seq_file *s, void *chip_data, int debug_data_type)
{
    uint16_t mutual_num;
    uint16_t self_num;
    uint16_t width;
...
static void hx83102e_baseline_read(struct seq_file *s, void *chip_data)
{
    hx83102e_read_debug_data(s, chip_data, DEBUG_DATA_BASELINE);
    hx83102e_read_debug_data(s, chip_data, DEBUG_DATA_RAW);
    return;
}

static void hx83102e_delta_read(struct seq_file *s, void *chip_data)
{

    struct chip_data_hx83102e *chip_info;
    chip_info = (struct chip_data_hx83102e *)chip_data;
    hx83102e_read_debug_data(s, chip_data, DEBUG_DATA_DELTA);
#ifdef CONFIG_OPPO_TP_APK
    if (chip_info->debug_mode_sta) {
        hx83102e_read_debug_data(s, chip_data, DEBUG_DATA_DOWN);
    }
#endif // end of CONFIG_OPPO_TP_APK
    return;
}

static void hx83102e_main_register_read(struct seq_file *s, void *chip_data)
{
    return;
}

//Reserved node
static void hx83102e_reserve_read(struct seq_file *s, void *chip_data)
{
    return;
}

static fw_update_state hx83102e_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
...
static int hx83102e_get_usb_state(void)
{
...
static int hx83102e_reset_gpio_control(void *chip_data, bool enable)
{
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
    if (gpio_is_valid(chip_info->hw_res->reset_gpio)) {
        TPD_INFO("%s: set reset state %d\n", __func__, enable);
    #ifdef HX_RST_PIN_FUNC
        hx83102e_resetgpio_set(g_chip_info->hw_res, enable);
    #endif
...
static void hx83102e_set_touch_direction(void *chip_data, uint8_t dir)
{
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
...
static uint8_t hx83102e_get_touch_direction(void *chip_data)
{
    struct chip_data_hx83102e *chip_info = (struct chip_data_hx83102e *)chip_data;
...
/*Himax_DB_Test Start*/
int hx83102e_freq_point = 0;
void hx83102e_freq_hop_trigger(void *chip_data)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
    ...
    hx83102e_freq_point = 1 - hx83102e_freq_point;
    if (hx83102e_freq_point) {//hop to frequency 130K
        do {
            if (rtimes > 10) {
...
static struct oppo_touchpanel_operations hx83102e_ops = {
    .ftm_process      = hx83102e_ftm_process,
    .get_vendor       = hx83102e_get_vendor,
    .get_chip_info    = hx83102e_get_chip_info,
    .reset            = hx83102e_reset,
    .power_control    = hx83102e_power_control,
    .fw_check         = hx83102e_fw_check,
    .fw_update        = hx83102e_fw_update,
    .trigger_reason   = hx83102e_trigger_reason,
    .get_touch_points = hx83102e_get_touch_points,
    .get_gesture_info = hx83102e_get_gesture_info,
    .mode_switch      = hx83102e_mode_switch,
    .exit_esd_mode    = hx83102e_exit_esd_mode,
    //.resume_prepare = hx83102e_resume_prepare,
    .get_usb_state    = hx83102e_get_usb_state,
    .black_screen_test = hx83102e_black_screen_test,
    .reset_gpio_control = hx83102e_reset_gpio_control,
    .set_touch_direction    = hx83102e_set_touch_direction,
    .get_touch_direction    = hx83102e_get_touch_direction,
/*Himax_DB_Test Start*/
    .freq_hop_trigger = hx83102e_freq_hop_trigger,
/*Himax_DB_Test End*/
};

static struct himax_proc_operations hx83102e_proc_ops = {
    .auto_test     = hx83102e_auto_test,
    .himax_proc_register_write =  hx83102e_proc_register_write,
    .himax_proc_register_read =  hx83102e_proc_register_read,
    .himax_proc_diag_write =  hx83102e_proc_diag_write,
    .himax_proc_diag_read =  hx83102e_proc_diag_read,
    .himax_proc_DD_debug_read =  hx83102e_proc_DD_debug_read,
    .himax_proc_DD_debug_write =  hx83102e_proc_DD_debug_write,
    .himax_proc_FW_debug_read =  hx83102e_proc_FW_debug_read,
    .himax_proc_reset_write =  hx83102e_proc_reset_write,
    .himax_proc_sense_on_off_write =  hx83102e_proc_sense_on_off_write,
...
static struct debug_info_proc_operations debug_info_proc_ops = {
    ...
    .delta_read    = hx83102e_delta_read,
    .baseline_read = hx83102e_baseline_read,
    .main_register_read = hx83102e_main_register_read,
    .reserve_read = hx83102e_reserve_read,
};
...
{
    hx83102e_mode_switch(chip_data, MODE_GAME, on_off);
}
...
static void himax_apk_earphone_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83102e *chip_info;
    chip_info = (struct chip_data_hx83102e *)chip_data;
    hx83102e_mode_switch(chip_data, MODE_HEADSET, on_off);
...
static void himax_apk_charger_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83102e *chip_info;
    chip_info = (struct chip_data_hx83102e *)chip_data;
    hx83102e_mode_switch(chip_data, MODE_CHARGE, on_off);
							...
							size_t len, loff_t *pos)
{
	...
	ret = snprintf(buf_tmp, sizeof(buf_tmp),
				"[VENDOR] = TXD, [IC] = HX83102E, [FW_VER] = 0x%02X\n", g_touch_ver);
...
* Version: 2020020701
*/
int __maybe_unused hx83102e_tp_probe(struct spi_device *spi)
{
    struct chip_data_hx83102e *chip_info = NULL;
    ...
    chip_info = kzalloc(sizeof(struct chip_data_hx83102e), GFP_KERNEL);
    if (chip_info == NULL) {
    ...
    chip_info->syna_ops = &hx83102e_proc_ops;
    ...
    //step7:create hx83102e related proc files
...
int __maybe_unused hx83102e_tp_remove(struct spi_device *spi)
{
...
static int hx83102e_i2c_suspend(struct device *dev)
{
...
static int hx83102e_i2c_resume(struct device *dev)
{
    struct touchpanel_data *ts = dev_get_drvdata(dev);
...
#ifdef CONFIG_FB
    .suspend = hx83102e_i2c_suspend,
    .resume = hx83102e_i2c_resume,
#endif
};


static struct spi_driver himax_common_driver = {
    .probe      = hx83102e_tp_probe,
    .remove     = hx83102e_tp_remove,
    .id_table   = tp_id,
    .driver = {
        .name = TPD_DEVICE,
        .owner = THIS_MODULE,
