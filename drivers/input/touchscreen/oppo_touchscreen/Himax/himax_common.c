...
static int himax_proc_diag_open_func(struct seq_file *s, void *v)
{
    struct touchpanel_data *ts = s->private;
    struct himax_proc_operations *syna_ops;
    int ret = 0;
    ...
    ret = syna_ops->himax_proc_diag_read(s, v);
    return ret;
}
...
{
    return single_open(file, himax_proc_diag_open_func, PDE_DATA(inode));
}

static const struct file_operations himax_proc_diag_ops = {
    .owner = THIS_MODULE,
    .open  = himax_proc_diag_open,
...
int himax_create_proc(struct touchpanel_data *ts, struct himax_proc_operations *syna_ops)
{
    ...
    prEntry_tmp = proc_create_data("tp_selftest", 0666, NULL, &tp_auto_test_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc/tp_selftest proc entry, %d\n", __func__, __LINE__);
    }
