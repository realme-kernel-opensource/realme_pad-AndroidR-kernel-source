...
static void init_project_version(void)
{
    ...
    if(is_new_cdt()){
        ...
        if(oppo_info){
            remove_proc_entry("oppoVersion/productName", NULL);
            pr_err("remove proc productName\n");
            remove_proc_entry("oppoVersion/prjName", NULL);
            pr_err("remove proc prjName\n");
        }
        if(oppo_info_temp){
            remove_proc_entry("oplusVersion/productName", NULL);
            pr_err("remove proc productName\n");
...
static int __init oppo_project_init(void)
{
    ...
    p_entry = proc_create_data("productName", S_IRUGO, oppo_info, &project_info_fops, UINT2Ptr(RF_INFO));
    if (!p_entry)
        goto error_init;
    ...
    p_entry = proc_create_data("productName", S_IRUGO, oppo_info_temp, &project_info_fops, UINT2Ptr(RF_INFO));
    if (!p_entry)
        goto error_init;
