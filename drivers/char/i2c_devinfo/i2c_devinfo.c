...
{
	pr_info("20200115-buf=%s\n",buf);

	pr_info("20200115-(sizeval+sizebuf)=%d\n",strlen(i2c_devinfo_dev->val)+strlen(buf));

	/*dingjun modify for fix CAP-2589*/
	if( strlen(i2c_devinfo_dev->val)+strlen(buf) < 255 ) {
		strcat(i2c_devinfo_dev->val, buf);
	} else {
		pr_err("%s,%d ERROR:array size exceeded!\n",__func__, __LINE__);
		return -1;
	}
